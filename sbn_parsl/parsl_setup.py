...
def create_provider_by_hostname(cfg: Config, local: bool = False):
    ...
    if cfg.site.monitor_cmd:
        ...
        monitor_cmd = (
            ...
            'if [ -z "$NUM_NODES" ] && [ -f "$PBS_NODEFILE" ]; then NUM_NODES=$(sort -u "$PBS_NODEFILE" | wc -l); fi && '
            'if [ -n "$NUM_NODES" ] && [ "$NUM_NODES" -gt 0 ]; then '
            f'mpiexec -n "$NUM_NODES" --ppn 1 {cfg.site.monitor_cmd} > lar_mon_startup.log 2>&1 </dev/null & '
            'else '
